function interactive_6dof_robot()
    ...
    % --- RRT* PLANNER WITH L2 (EUCLIDEAN) NORM ---
    function path = planRRTStar(q_start, q_goal, max_nodes, step_size, goal_bias)
        ...
        for iter = 1:max_nodes
            ...
            % Find Nearest Node Using L2 Norm
            diffs_nn = mod(tree.q - q_rand + 180, 360) - 180;
            dists = sqrt(sum(diffs_nn.^2, 2));
            [~, nearest_idx] = min(dists);

            q_near = tree.q(nearest_idx, :);
            dir = mod(q_rand - q_near + 180, 360) - 180;
            dist = norm(dir, 2); % L2 (Euclidean) Norm for vector direction limiting
            ...
            if ~checkCollisions(q_new)
                % RRT* Radius Search (L2 Norm)
                diffs_all = mod(tree.q - q_new + 180, 360) - 180;
                dists_all = sqrt(sum(diffs_all.^2, 2));
                near_indices = find(dists_all <= search_radius);

                % Optimize Parent
                best_parent = nearest_idx;
                min_cost = tree.cost(nearest_idx) + norm(mod(q_new - tree.q(nearest_idx,:) + 180, 360) - 180, 2);

                for i = 1:length(near_indices)
                    near_idx = near_indices(i);
                    c_near = tree.cost(near_idx) + norm(mod(q_new - tree.q(near_idx,:) + 180, 360) - 180, 2);
                    if c_near < min_cost
                ...
                for i = 1:length(near_indices)
                    ...
                    c_rewire = min_cost + norm(mod(tree.q(near_idx,:) - q_new + 180, 360) - 180, 2);
                ...
                % Goal detection logic using L2
                goal_diff = mod(q_new - q_goal + 180, 360) - 180;
                if norm(goal_diff, 2) < step_size
                    tree.q = [tree.q; q_goal];
                    tree.parent = [tree.parent; new_idx];
                    tree.cost = [tree.cost; min_cost + norm(goal_diff, 2)];
